...
<div class="row mb-3">
    ...
    <div class="col-auto">
        <a href="{{ route('routes.generate') }}" class="btn text-white" style="background-color: #A80000;">
            <i class="ti ti-map-pin"></i> Generate Routes
        </a>
        <form action="{{ route('routes.reset') }}" method="POST" class="d-inline">
            @csrf
...
@foreach ($groupedRoutes as $date => $routes)
    <div class="card my-4">
        <div class="card-header">
            Routes for {{ \Carbon\Carbon::parse($date)->format('F j, Y') }}
        </div>
        <div class="card-body">
                                    ...
                                    @foreach ($route['stops'] as $stop)
                                        <li class="list-group-item sortable-stop d-flex justify-content-between align-items-start flex-column"
                                            ...
                                            <div class="me-2">
                                                ...
                                                {{ $stop['contact'] ?? '' }}<br>
                                                Orders: {{ implode(', ', $stop['order_numbers']) }}<br>
