...
<script>
  ...
  function renderDepositMethodFields(method){
    ...
    @if(count($firstServePaymentMethods) > 0)
        ipmSavedCardSection.classList.remove('d-none'); // hide saved card section
    @endif
  }
...
// Validate ipmPercentValue: must be between 0 and 100
$('#ipmPercentValue').on('input', function () {
