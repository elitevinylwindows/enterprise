<form action="{{ route('prices.update', $price->id) }}" method="POST">
    ...
    <div class="modal-body row">
        <div class="mb-3 col-md-6">
            ...
            <input type="text" name="material_name" class="form-control" value="{{ $price->material_name }}" required>
        </div>

        <div class="mb-3 col-md-6">
            <label for="description" class="form-label">Description</label>
            <textarea name="description" class="form-control" required>{{ $price->description }}</textarea>
        </div>

        <div class="mb-3 col-md-6">
    <label for="unit" class="form-label">Unit</label>
    <input type="text" name="unit" id="unit" class="form-control" value="{{ $price->unit ?? '' }}">
</div>


        <div class="mb-3 col-md-6">
            <label for="vendor" class="form-label">Vendor</label>
            <input type="text" name="vendor" class="form-control" value="{{ $price->vendor }}">
        </div>
        ...
        <div class="mb-3 col-md-6">
            ...
            <select name="sold_by" id="sold_by" class="form-control">
                ...
                @foreach(['By BOX', 'By PCs', 'By Roll', 'By BND', 'By Tube', 'By Spool'] as $option)
                    <option value="{{ $option }}" {{ $price->sold_by == $option ? 'selected' : '' }}>
                        {{ $option }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3 col-md-6">
            ...
            <input type="text" name="lin_pcs" id="lin_pcs" class="form-control" value="{{ $price->lin_pcs }}" readonly>
        </div>
    </div>
